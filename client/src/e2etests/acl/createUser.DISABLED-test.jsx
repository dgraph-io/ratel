...
test("Should be able to create and delete a user", async () => {
    ...
    await expect(page.$eval(addBtnSelector, (btn) => btn.textContent)).resolves.toBe("Add User")
    ...
    await expect(page.$eval(deleteUserSelector, (btn) => btn.textContent)).resolves.toBe(
        "Delete User",
    )
    ...
    await expect(dialogPromise).resolves.toBe(`Are you sure you want to delete "${userId}"?`)

    await expect(waitForElementDisappear(page, addedUserRowSelector)).resolves.toBe(true)
})
