...
export const DGRAPH_SERVER = process.env.TEST_DGRAPH_SERVER || "http://localhost:8080"
...
export const waitUntil = async (fn, { timeout = 20000, step = 20, page } = {}) => {
...
export const waitForElement = async (page, query, { timeout = 10000 } = {}) => {
    try {
        ...
        throw new Error(`Timeout waiting for element "${query}". Error: ${err.message}`)
    }
}

export const waitForElementDisappear = async (page, query, { timeout = 10000 } = {}) => {
    try {
...
export const easyUid = () => `${Date.now() % 9973}_${Math.round(Math.random() * 9967)}`

export const getElementText = async (page, query) => await page.$eval(query, (el) => el.textContent)
...
export const findElementWithText = async (page, query, textContent) => {
    ...
    const texts = await page.$$eval(query, (elements) => elements.map((el) => el.textContent))
