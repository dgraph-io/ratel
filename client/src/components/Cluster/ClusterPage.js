...
export default function ClusterPage() {
    ...
    const { instanceHealth, isAuthorized, clusterState } = useSelector((state) => state.cluster)
    ...
    if (!isAuthorized) {
        return (
            <div className="alert alert-danger" style={{ margin: "20px 40px" }}>
                You need to login as a <strong>guardians group</strong> member to view Cluster
                State.
            </div>
        )
    }
    ...
    const getHealthDot = (addr) => {
        ...
        return (
            <div
                className={health.status === "healthy" ? "health healthy" : "health dead"}
    ...
    const renderGroups = (groups) => {
        ...
        const compareTablets = (a, b) => {
            ...
            const cmpSpace = -cmp(parseFloat(getSpace(a[1])), parseFloat(getSpace(b[1])))
        ...
        const renderGroup = (key, g) => {
            ...
            const andMore = tablets.length > MAX_TABLETS ? tablets.length - MAX_TABLETS - 1 : 0
            ...
            return (
                ...
                >
                    ...
                    <div className="nodes">{Object.values(g.members || {}).map(renderNode)}</div>
