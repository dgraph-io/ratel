...
export default function BackupsView(props) {
    ...
    const dgraphUrl = useSelector((state) => state.connection.serverHistory[0].url)

    const backupConfig =
        useSelector((state) => state.backup && state.backup.config) || DEFAULT_BACKUP_CONFIG

    const backupsList = useSelector((state) => state.backup && state.backup.backups) || []
    ...
    const renderToolbar = () => {
        return (
            <div className="btn-toolbar" key="buttonsDiv">
                <button className="btn btn-primary btn-sm" onClick={() => setBackupModal(true)}>
                    Create Backup
                </button>
            </div>
        )
            ...
            },
            {
                ...
                formatter: ({ value }) => (
                    <span title={value.path}>
                        <Badge variant="secondary">{value.type}</Badge> {value.path}
                    </span>
                ),
            },
            {
            ...
            },
            {
                ...
                formatter: ({ value }) => {
                    ...
                    if (value.error) {
                        ...
                        return (
                            <span title={msg}>
                                <Badge variant="danger">{err.name || "Error"}</Badge> {msg}
                            </span>
                        )
                    }
                    return (
