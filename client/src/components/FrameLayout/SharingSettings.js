...
export default function SharingSettings({ title, query }) {
    ...
    const serverAddr = useSelector((state) => state.connection?.serverHistory?.[0]?.url)

    const serverAddrPart = !includeAddr ? "" : `&addr=${encodeURIComponent(serverAddr)}`
        ...
        <div className="sharingSettings">
            <Form.Group controlId="frameUrlText" className="urlRow">
                <Form.Control
                    ...
                    onChange={() => 0}
                />
                <button className="btnCopy" onClick={onCopyToClipboard} title="Copy to Clipboard">
                    {showCopied ? (
                        "Copied!"
                    ) : showCopyError ? (
                        "Error!"
