...
export default function FrameSession({ frame, tabResult }) {
    const { panelMinimized, panelHeight, panelWidth } = useSelector((store) => store.ui)

    const dispatch = useDispatch()

    const handlePanelResize = (panelSize) => dispatch(setPanelSize(panelSize))
    const handleSetPanelMinimized = (minimized) => dispatch(setPanelMinimized(minimized))
    ...
    const isSchemaGraph = frame.action === "query" && /^[ \t\n]*schema[{ \t\n].*/.test(frame.query)

    const graphParser =
        frame.action === "query" && getGraphParser(tabResult && tabResult.response, isSchemaGraph)

    const forceReRender = () => {
        const graph = graphParser.getCurrentGraph()
        setGraphUpdateHack(`${Date.now()} ${graph.edges.length} ${graph.nodes.length}`)
    }
    ...
    return (
        <React.Fragment>
            ...
            />
            <EntitySelector graphLabels={graph.labels} onPredicateHovered={setHoveredPredicate} />
        </React.Fragment>
    )
}
