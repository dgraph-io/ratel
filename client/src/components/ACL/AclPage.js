...
function SortableGrid({ rows, selection: [selectedName, setSelectedName], columns }) {
    ...
    const getGridData = memoize((rows, usersSortColumn, usersSortDirection) => {
        ...
        rows = rows.map((row) => {
            return {
                ...row,
                totalPredicateCount:
                    (row.acl && row.acl.length && row.acl.map((acl) => acl.predicate).length) || 0,
            }
        })
...
export default function AclPage() {
    ...
    }, [setFetchState, setLastUpdated, setUsers, setGroups, setPredicates, setLoadingError])
    ...
    const renderUsersToolbar = () => {
        return (
            <div className="btn-toolbar" key="buttonsDiv">
                <button className="btn btn-primary btn-sm" onClick={handleNewUserClick}>
                    Add User
                </button>
    ...
    const renderGroupsToolbar = () => (
        <div className="btn-toolbar schema-toolbar" key="buttonsDiv">
            <button className="btn btn-primary btn-sm" onClick={() => setModal("addGroup")}>
                Add Group
            </button>
    ...
    const leftToolbar = leftTab === "users" ? renderUsersToolbar() : renderGroupsToolbar()
    const leftGrid =
        leftTab === "users" ? (
            <SortableGrid
        ...
        if (isAclError()) {
            return (
                <div className="alert alert-danger">
                    You need to login as a <strong>guardians group</strong> member to see ACL
                    settings.
                </div>
            )
        }
