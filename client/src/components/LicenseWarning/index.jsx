...
export default function LicenseWarning() {
    ...
    const licenseWarningDismissedTs = currentServer.licenseWarningDismissedTs || -1
    const { clusterState } = useSelector((state) => state.cluster)

    const license = clusterState?.license

    if (!license || moment.duration(Date.now() - licenseWarningDismissedTs) < NAG_INTERVAL) {
        return null
    }

    const timeLeft = moment.duration(license.expiryTs * 1000 - Date.now())

    if (timeLeft > EXPIRES_SOON_WINDOW || (timeLeft < 0 && timeLeft.abs() > HAS_EXPIRED_WINDOW)) {
        return null
    }
    ...
    if (license.enabled && timeLeft < EXPIRES_SOON_WINDOW) {
        return (
            <div className="license-warning">
                Your enterprise license will expire <strong>{timeLeft.humanize(true)}</strong> from
                now. To continue using enterprise features after{" "}
                <strong>{timeLeft.humanize()}</strong> from now, apply a valid license. To get a new
                license, contact us at{" "}
                <a href="https://dgraph.io/contact" target="_blank" rel="noopener noreferrer">
                    https://dgraph.io/contact
                </a>
                .
                <button className="dismiss" onClick={() => dispatch(dismissLicenseWarning())}>
                    dismiss
                </button>
            </div>
        )
    ...
    return (
        <div className="license-warning expired">
            Your enterprise license has expired and enterprise features are disabled. To continue
            using enterprise features, apply a valid license. To receive a new license, contact us
            at{" "}
            <a href="https://dgraph.io/contact" target="_blank" rel="noopener noreferrer">
                https://dgraph.io/contact
            </a>
