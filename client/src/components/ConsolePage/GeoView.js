...
export default function GeoView({ results }) {
    ...
    const parseResults = (results) => {
        const data =
            results && results.response && results.response.data ? results.response.data : {}
        const parsedResults = []
    ...
    const renderPolygon = (record, polygonColor = "blue") => {
        const location = record[locationField]
        const points = location.coordinates.map((a) => a.map((c) => c.slice().reverse()))
        ...
        <>
            <div className="map-wrapper">
                ...
                <div className="text-right pr-5">
                    <Button variant="light" onClick={handleShow} className="options-button mt-2">
