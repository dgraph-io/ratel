...
export default class SchemaPredicateForm extends React.Component {
    ...
    getNameErrorMsg = () => {
        ...
        for (let char of name) {
            if (char.charCodeAt(0) <= 32 || predicateErrorStrings.indexOf(char) >= 0) {
    ...
    getTokenizerErrors = () => {
        ...
        if (predicate.type === "string") {
            ...
            if (hasExact && hasHash) {
                warnings.push("'exact' and 'hash' index types shouldn't be used together")
            }
        ...
        if (predicate.type === "datetime" && predicate.index && arr.length === 0) {
            // NOTE: predicate is string && indexed => tokenizer is required
            warnings.push("At least 1 tokenizer should be selected")
        }
    ...
    */
    ...
    handleNameChange = this.useEventTargetValue(this.handlePropertyChange("predicate"))

    handleCountChange = this.useEventTargetChecked(this.handlePropertyChange("count"))

    handleReverseChange = this.useEventTargetChecked(this.handlePropertyChange("reverse"))
    ...
    handleUpsertChange = this.useEventTargetChecked(this.handlePropertyChange("upsert"))
                    ...
                    <Col sm={9}>
                        {tokenizers.map((tok) => (
                            <Form.Check
                                ...
                                id={"check-" + tok}
                                label={tok}
                                onChange={(event) => this.handleTokenizerChange(tok, event)}
                            />
                        ))}
                        {tokenizerErrorList}
                        {helpDivIndexing}
        ...
        const predicateName = (
            <Col sm={9}>
                <Form.Control
                    ...
                    disabled={!createMode}
                />
                {createMode && predicate.predicate.length > 1 && nameErrorMsg ? (
