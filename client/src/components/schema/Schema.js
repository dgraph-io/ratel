...
export default class Schema extends React.Component {
    ...
    fetchSchema = async () => {
        ...
        try {
            const schemaResponse = await executeQuery("schema {}", { action: "query" })
    ...
    executeSchemaQuery = async (query, action) => {
        try {
            ...
            throw new Error(errorText || (error.toString && error.toString()) || error)
        }
    }
    ...
    renderToolbar = () => {
        ...
        return (
            <div className="btn-toolbar schema-toolbar" key="buttonsDiv">
                ...
                {leftPaneTab === "types" && (
                    <button className="btn btn-primary btn-sm" onClick={this.handleNewTypeClick}>
                        Add Type
                    </button>
                )}
                ...
                >
                    <input type="radio" name="action" checked={leftPaneTab === "types"} readOnly />
                    &nbsp;Types
                </button>
                ...
                >
                    {fetchState === STATE_LOADING ? "Refreshing Schema..." : "Refresh Schema"}
                </button>
                {!lastUpdated ? null : (
                    <span
                        style={{
                    ...
                    >
                        Updated&nbsp;
                        <TimeAgo date={lastUpdated} formatter={timeAgoFormatter} minPeriod={10} />
                    </span>
                )}
            </div>
        )
    ...
    render() {
        ...
        const isAccessError =
            errorMsg?.indexOf("rpc error: code = Unauthenticated desc = no accessJwt available") >=
            0
            ...
            fetchState !== STATE_ERROR ? null : (
                <div className="col-sm-12" style={{ flex: 0, margin: "32px 0 64px" }}>
                    <div className="alert alert-danger" role="alert">
                        ...
                        <button className="btn btn-secondary btn-sm" onClick={this.fetchSchema}>
                            Refresh Schema
                        </button>
                    </div>
                </div>
            ...
            !schema || this.isSchemaEmpty() ? (
                <div className="panel panel-default" key="dataDiv">
                    <div className="panel-body">
                        There are no predicates in the schema. Click the button above to add a new
                        predicate.
                    </div>
                </div>
            ) : (
        ...
        const renderTypesTable = () =>
            !types.length ? (
                ...
                <TypesTable
                    ...
                    onChangeSelectedType={(t) => this.setState({ selectedTypeName: t && t.name })}
                />
            )

        const dataDiv = leftPaneTab === "predicates" ? renderSchemaTable() : renderTypesTable()
        ...
        const rightPane =
            leftPaneTab === "predicates" ? (
                ...
                <TypeProperties type={selectedType} onEdit={this.handleEditSelectedType} />
            ) : (
