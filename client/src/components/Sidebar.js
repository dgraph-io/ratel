...
export default function Sidebar({ currentMenu, currentOverlay, onToggleMenu }) {
    ...
    if (!currentServer || typeof currentServer !== 'object' || !currentServer.url) {
    ...
    const renderButton = ({ menuId, label, icon, fontAwesomeIcon, extraClassname, locked, key }) => {
        const className = currentMenu === menuId ? "link active" : "link"
        if (process.env.NODE_ENV !== "production") {
            console.log("Sidebar renderButton props:", { menuId, label, icon, fontAwesomeIcon, extraClassname, locked })
        }
        return (
    ...
    const renderConnectionString = () => {
        ...
        return (
            <div className={"connection-string " + errorStyle}>
                <HealthDot health={currentServer.health} version={currentServer.version} />
