...
store.subscribe(() => {
    ...
    if (!state.connection?.serverHistory) {
        console.warning("Redux State is not ready. Waiting for connection.serverHistory")
        return
    }
    setCurrentServerUrl(state.connection.serverHistory[0].url)
    setCurrentServerQueryTimeout(state.connection.serverHistory[0].queryTimeout || 20)
...
export default class AppProvider extends React.Component {
    ...
    onRehydrated = () => {
        ...
        if (hashParams.slashApiKey) {
            store.dispatch(setSlashApiKey(hashParams.addr || addrParam, hashParams.slashApiKey))
        }
