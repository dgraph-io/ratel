...
export class GraphParser {
    ...
    nameNode(nodeAttrs, regexStr) {
        ...
        if (aggrTerm !== "") {
            ...
            const fullName = regexStr ? getNodeLabel(nodeAttrs, new RegExp(regexStr, "i")) : ""
            return {
    ...
    processQueue = (regexStr = null, maxAdd = FIRST_RENDER_LIMIT) => {
        ...
        while (this.queue.length > 0) {
            ...
            for (let prop of Object.keys(obj.node).sort()) {
                ...
                } else if (typeof val === "object" && val && typeof val.uid === "string") {
            ...
            const { displayLabel, fullName } = this.nameNode(properties.attrs, regexStr)
            ...
            const edgeKey = [obj.src.id, uid, groupProperties.pred].filter((val) => val).join("-")
    ...
    collapseNode = (expansionUid) => {
        this.queue = this.queue.filter((el) => el.src.expansionNode !== expansionUid)
        ...
        this.edgesDataset.forEach((edge, key) => {
            if (uidsToRemove.has(edge.source.uid) || uidsToRemove.has(edge.target.uid)) {
