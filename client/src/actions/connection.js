...
import { FetchError, OK, QUERY_TIMEOUT_DEFAULT, Unknown } from "../lib/constants"
...
const SLASH_DOMAINS = [".app.thegaas.com", ".slash.dgraph.io", ".cloud.dgraph.io", "hypermode.host"]
...
export const checkNetworkHealth = async (dispatch, getState) => {
    ...
    try {
        ...
        try {
            ...
            dispatch(
                setMultiTenancyEnabled(url, ee_features.includes("multi_tenancy") ? true : false),
            )
...
export const checkAclState = async (dispatch, getState) => {
    ...
    try {
        ...
        const res = await helpers.executeQuery("{ q(func: uid(1)) { uid } }", { action: "query" })
